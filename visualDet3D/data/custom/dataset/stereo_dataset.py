...
class CustomStereoDataset(torch.utils.data.Dataset):
    ...
    def _reproject(self, P2: np.ndarray, transformed_label: List[KittiObj]) -> Tuple[List[KittiObj], np.ndarray]:
        ...
        if len(transformed_label) > 0:
            ...
            try:
                ...
                print('\n', bbox3d_origin.shape, len(transformed_label), len(label), label, transformed_label,
                      bbox3d_origin)
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            if abs(P2[0, 3]) < abs(
                    P3[0, 3]):  # not mirrored or swaped, disparity should base on pointclouds projecting through P2
                disparity = cv2.imread(os.path.join(self.preprocessed_path, 'training', 'disp', "P2%06d.png" % index),
                                       -1)
            else:  # mirrored and swap, disparity should base on pointclouds projecting through P3, and also mirrored
                disparity = cv2.imread(os.path.join(self.preprocessed_path, 'training', 'disp', "P3%06d.png" % index),
                                       -1)
